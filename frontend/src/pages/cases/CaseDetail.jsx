/**
 * GARUDA — Case Detail View (Page 3)
 * Route: /cases/:id
 * Shows full case details with visual timeline, accused list, seizures, and court status.
 */
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold" style={{ color: 'var(--color-garuda-50)' }}>
            Case: {caseData.fir_no}
          </h1>
          <p className="text-sm mt-1" style={{ color: 'var(--color-garuda-400)' }}>
            {caseData.police_stations?.name} • {caseData.case_date ? new Date(caseData.case_date).toLocaleDateString('en-IN') : '—'}
          </p>
        </div>
        {perms.canEditCase && (
          <Link
              ...
              <div key={stage} className="flex items-center flex-1">
                <div className="flex flex-col items-center flex-1">
                  <div
                    ...
                    style={{
                      background: isCurrent ? 'var(--color-accent-500)' : isActive ? 'var(--color-accent-500)' : 'var(--color-garuda-700)',
                ...
                {i < STAGES.length - 1 && (
                  <div
                    ...
                    style={{ background: i < currentStageIdx ? 'var(--color-accent-500)' : 'var(--color-garuda-700)' }}
                  />
                )}
              </div>
            );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-xl p-6 space-y-3" style={{ background: 'var(--color-garuda-800)', border: '1px solid var(--color-garuda-700)' }}>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <p style={{ color: 'var(--color-garuda-100)' }}>{caseData.fir_no}</p>
            </div>
            <div>
              <p className="text-xs" style={{ color: 'var(--color-garuda-500)' }}>Section of Law</p>
              <p style={{ color: 'var(--color-garuda-100)' }}>{caseData.section_of_law || '—'}</p>
            </div>
            <div>
              <p className="text-xs" style={{ color: 'var(--color-garuda-500)' }}>Station</p>
              <p style={{ color: 'var(--color-garuda-100)' }}>{caseData.police_stations?.name || '—'}</p>
            </div>
            <div>
              <p className="text-xs" style={{ color: 'var(--color-garuda-500)' }}>Date</p>
              <p style={{ color: 'var(--color-garuda-100)' }}>{caseData.case_date ? new Date(caseData.case_date).toLocaleDateString('en-IN') : '—'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl p-6 space-y-3" style={{ background: 'var(--color-garuda-800)', border: '1px solid var(--color-garuda-700)' }}>
          <h3 className="font-semibold" style={{ color: 'var(--color-garuda-200)' }}>
            Accused ({caseData.case_accused?.length || 0})
          </h3>
          {caseData.case_accused?.length > 0 ? (
            <ul className="space-y-2">
              {caseData.case_accused.map((ca) => (
                <li key={ca.id} className="flex items-center justify-between p-2 rounded" style={{ background: 'var(--color-garuda-900)' }}>
                  <div>
                    <p className="text-sm font-medium" style={{ color: 'var(--color-garuda-100)' }}>
                      {ca.offenders?.full_name || `Offender #${ca.offender_id}`}
                    </p>
                    <p className="text-xs" style={{ color: 'var(--color-garuda-400)' }}>
                      {ca.arrest_status}
                      {ca.arrest_date ? ` • Arrested: ${new Date(ca.arrest_date).toLocaleDateString('en-IN')}` : ''}
                    </p>
                  </div>
                  <Link
                    to={`/offenders/${ca.offender_id}/edit`}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {caseData.seizures.map((s) => (
              <div key={s.id} className="p-3 rounded-lg" style={{ background: 'var(--color-garuda-900)' }}>
                <p className="text-xs" style={{ color: 'var(--color-garuda-400)' }}>
                  {s.seizure_date ? new Date(s.seizure_date).toLocaleDateString('en-IN') : 'No date'}
                </p>
                {s.contraband_kg && <p className="text-sm" style={{ color: 'var(--color-warning-400)' }}>{s.contraband_kg} Kg contraband</p>}
                {s.cash_amount > 0 && <p className="text-sm" style={{ color: 'var(--color-success-400)' }}>₹{Number(s.cash_amount).toLocaleString('en-IN')} cash</p>}
                {s.vehicles_count > 0 && <p className="text-sm" style={{ color: 'var(--color-info-400)' }}>{s.vehicles_count} vehicle(s)</p>}
              </div>
            ))}
          </div>
        </div>
